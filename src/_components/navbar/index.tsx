    ...
    return (
        <Navbar fluid rounded className="bg-primary dark:bg-ONYX">
            ...
            <div className="flex gap-[10px]">
                ...
                {name && (
                    <Link to="/curation-select" className="text-white-200 flex items-center font-bold">
                        큐레이션 생성
                    </Link>
                )}

                {name && (
                    <Link className="text-white-200 flex items-center font-bold" to="/mypage">
                        {name}
                    </Link>
                )}
                {!name && (
                    <Link to="/login" className="text-white-200 flex items-center font-bold">
                        로그인
                    </Link>
                )}
                <DarkThemeToggle

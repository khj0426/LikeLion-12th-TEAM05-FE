...
export const loginSchema = z.object({
    ...
    password: z
        ...
        .regex(/[!@#$%^&*(),.?":{}|<>]/, '비밀번호에는 최소 한 개의 특수문자가 포함되어야 합니다.'),
})
    ...
    return (
        <section className="flex flex-col gap-4 p-5">
            <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto bg-white rounded-xl shadow-lg">
                ...
                <p className="mt-6 text-2xl text-SLATE font-bold">로그인 하여 즐겨보세요</p>

                <div style={{ margin: '0 auto' }}>
                    <Input placeholder="이름" id="name" name="name" value={formData.name} onChange={onChange} className="w-[350px]" />
                    <p className="mt-2 text-sm text-red-600 dark:text-red-500 flex justify-center">
                        <span className="font-medium">{error?.flatten().fieldErrors['name']}</span>
                    </p>
                    <Input placeholder="이메일" id="email" name="email" value={formData.email} onChange={onChange} className="w-[350px]" />
                    <p className="mt-2 text-sm text-red-600 dark:text-red-500 flex justify-center">
                        <span className="font-medium">{error?.flatten().fieldErrors['email']}</span>
                    </p>
                    <Input placeholder="비밀번호" id="password" name="password" value={formData.password} onChange={onChange} className="w-[350px]" />
                    <p className="mt-2 text-sm text-red-600 dark:text-red-500 flex justify-center">
                        <span className="font-medium">{error?.flatten().fieldErrors['password']}</span>
                    </p>
